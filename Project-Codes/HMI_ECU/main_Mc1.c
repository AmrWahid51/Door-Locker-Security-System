/*
 ...
 Name        : main_Mc1.c
 Author      : Amr Wahid
 Description : Code for user to interface with a vault
 Date        : 1/7/2023
 ================================================================================================
 */
#include "lcd.h"
...
void testing_password(void)
{
	...
	if (choice == '+')
	{
		Timer1_ConfigType config_timer = {0,58594,CLOCK_1024,COMPARE}; // setting timer1 init (will be changed)
		UART_sendByte(open);
...
void time_handler (void)
{
	...
	if(count == 2)
	{
		...
		Timer1_ConfigType config_timer = {0,23438,CLOCK_1024,COMPARE};
		Timer1_init(&config_timer);
	}
	if (count == 3)
	{
		...
		Timer1_ConfigType config_timer = {0,58594,CLOCK_1024,COMPARE};
		Timer1_init(&config_timer);
